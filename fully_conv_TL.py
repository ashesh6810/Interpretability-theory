...
savenc_for_activations(Act,output_training,2,num_epochs,7,num_samples,64,192,96,'BNN_Activations'+str(trainN)+'sample_size'+str(num_samples)+'_dt'+str(lead)+'.nc')
...
hdf5storage.write(matfiledata, '.', 'BNN_Weights'+str(trainN)+'sample_size'+str(num_samples)+'_dt'+str(lead)+'.mat', matlab_compatible=True)
...
mylist = [2]
for k in mylist:
  fileList_train.append ('/glade/scratch/asheshc/theory-interp/QG/Transfer-Learning/case'+str(k)+'/PSI_output.nc')
...
torch.save(net.state_dict(), './TLNN_layer_7_6.pt')

print('TLNN Model Saved')

savenc_for_activations(Act,output_training,2,num_epochs,7,num_samples,64,192,96,'TLNN_Activations_layer_7_6'+str(trainN)+'sample_size'+str(num_samples)+'_dt'+str(lead)+'.nc')
...
matfiledata[u'hidden_weights'] = hidden_weights_network
matfiledata[u'final_layer_weights'] = final_weights_network
hdf5storage.write(matfiledata, '.', 'TLNN_Weights_layer_7_6'+str(trainN)+'sample_size'+str(num_samples)+'_dt'+str(lead)+'.mat', matlab_compatible=True)
...
savenc(autoreg_pred, lon, lat, 'predicted_psi_TL_doublejet_layer_7_6_lead'+str(lead)+'.nc')
savenc(psi_test_label_Tr, lon, lat, 'truth_psi_TL_doublejet_layer_7_6_lead'+str(lead)+'.nc')
